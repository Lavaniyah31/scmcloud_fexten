    ...
    public function most_popular_product()
    {
        ...
        if ($this->db->table_exists('site_analytics')) {
            ...
            $result = $this->db->select('pi.*, pc.*, b.brand_name, sa.clicks')
                ...
                ->join('product_category as pc', 'pc.category_id = pi.category_id', 'left')
        ...
        // Fallback: Get products from order_details if site_analytics is empty
        $product_ids = $this->db->query('SELECT DISTINCT product_id FROM `order_details`')->result_array();
        $product_ids = array_column($product_ids, 'product_id');
        
        if (!empty($product_ids)) {
            $result = $this->db->select('pi.*, pc.*, b.brand_name')
